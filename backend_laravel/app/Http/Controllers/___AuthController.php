...
class AuthController extends Controller
{
    ...
    public function teste1  () {
      Session::put('nome_usuario', '10');
      Session::put('token_usuario', '20');
      Session::put('teste_usuario', '30');
    ...
    public function verificarLogado  () {
      ...
      if (Auth::check()) { 
        ...
        $detalhesUsuario = Session::get('nomeUsuarioLogado') . '^' .
                           Session::get('tokenUsuarioLogado');
      }

      if ( Session::has('nomeUsuarioLogado')  )
        $detalhesUsuario = Session::get('nomeUsuarioLogado') . '^' .
                           Session::get('tokenUsuarioLogado');


//$detalhesUsuario = Session::get('teste_usuario').'--'.
//Session::get('nome_usuario');
    ...
    public function novoUsuario  (Request $request) {
      ...
      Session::put('nomeUsuarioLogado', Auth::user()->name);
      Session::put('tokenUsuarioLogado', csrf_token());

      return redirect()->route('verificarLogado');

/*
    if (Auth::attempt(['email' => $request->email, 'password' => $request->password], true)) { 
        Session::put('nome_usuario', 'aaa1'.Auth::user()->name);
        Session::put('token_usuario', 'ccc'.csrf_token());
    }
      return view('auth.session');
...
//    session()->invalidate();
 ...
 //if (Auth::attempt(['email' => $request->email, 'password' => $request->password], true)) {
    //$resOK = array('nome'=>Auth::user()->name);
    //session()->regenerate();
  //}
  //else {
    //$resOK = array('nome'=>'NAOLOGADO');
  //}
...
//dd('ljkdsljkdsljk');
      ...
      return view('auth.teste')->with('nome', Auth::user()->name);
    }
